...
public class Attacker3 : MonoBehaviour
{
    ...
    public bool allowedToShoot = false; // STUB
    ...
    [Space]
    [Range(0f, 10f)]
    [SerializeField] float inaccuracyFluctuationAmount = 5f; // 5f
    [Range(1, 15)]
    [SerializeField] int GaussLoops = 3; // 3;
    #endregion  
    ...
    private void Start()
    {
        //targetScript = targetT.gameObject.GetComponent<Target>();                 // v1, Player is the Target
        targetScript = targetT.gameObject.transform.parent.GetComponent<Target>(); // v2, Target CHILD of Player GameObject

        //shootingPos = transform; // WAS USED to set shootingPos as Transform
    }
    ...
    // Adjusts the Projectile Fly Arc to be more realistic looking
    ...
    private Vector3 ModifyAccuracy(Vector3 targetPos, float distance, float inaccuracyMod)
    {
        ...
        float maxInaccuracy = inaccuracyFluctuationAmount; // 5f
        float minRange = -maxInaccuracy; // -3f;
        int gaussLoops = GaussLoops; // 3;
